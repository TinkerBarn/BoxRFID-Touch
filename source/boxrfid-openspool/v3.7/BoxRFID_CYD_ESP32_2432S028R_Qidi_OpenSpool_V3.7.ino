...
static const char* APP_VERSION = "V3.7";
...
enum QidiPrinterModel : uint8_t { QIDI_MODEL_PLUS4 = 0, QIDI_MODEL_Q2 = 1 };
...
static const uint8_t MAX_MANUFACTURERS = 50;
static const uint8_t LEGACY_MAX_MANUFACTURERS = 24;
...
enum KeyboardMode : uint8_t { KB_UPPER=0, KB_LOWER=1, KB_NUM=2, KB_HEX=3 };
...
static const char* qidiPrinterModelLabel() {
  return (qidiPrinterModel == QIDI_MODEL_Q2) ? LTXT(TXT_QIDI_Q2) : LTXT(TXT_QIDI_PLUS4);
}

static uint8_t getSetupPageCount() {
  return (currentTagMode == TAGMODE_OPENSPOOL) ? 4 : 3;
}
...
static bool screensaverTouchPresent() {
  // Important: tirqTouched() alone is NOT sufficient - the IRQ pin can
  // stay LOW because of noise and produce false positives.
  // Use the same AND logic as in getTouchRaw().
...
static const char* currentMatPrefsNs() {
  if (currentTagMode == TAGMODE_QIDI) return (qidiPrinterModel == QIDI_MODEL_Q2) ? PREF_NS_MAT_QIDI_Q2 : PREF_NS_MAT_QIDI;
  return PREF_NS_MAT_OS;
}

static const char* currentMfgPrefsNs() {
  if (currentTagMode == TAGMODE_QIDI) return (qidiPrinterModel == QIDI_MODEL_Q2) ? PREF_NS_MFG_QIDI_Q2 : PREF_NS_MFG_QIDI;
...
static void loadMaterials() {
  ...
  if (!anyStored) {
    prefs.begin(currentMatPrefsNs(), true);
    for (uint8_t i = 1; i <= MAX_MATERIALS; i++) {
      char key[5];
      ...
      if (s.length() > 0) {
        ...
        gMaterials[i].nozzleMin = prefs.getUShort(key, 0);
        snprintf(key, sizeof(key), "x%02u", i);
        gMaterials[i].nozzleMax = prefs.getUShort(key, 0);
        snprintf(key, sizeof(key), "b%02u", i);
        gMaterials[i].bedMin = prefs.getUShort(key, 0);
        snprintf(key, sizeof(key), "d%02u", i);
        gMaterials[i].bedMax = prefs.getUShort(key, 0);
      }
    }
    prefs.end();
    if (anyStored) saveListBlob(currentMatPrefsNs(), gMaterials, sizeof(gMaterials));
  }
  for (uint8_t i = 1; i <= MAX_MATERIALS; i++) {
    if (gMaterials[i].active) {
...
static void loadVariants() {
  ...
  if (!anyStored) {
    prefs.begin(currentVarPrefsNs(), true);
    for (uint8_t i = 1; i <= MAX_VARIANTS; i++) {
      char key[12];
      snprintf(key, sizeof(key), "var%u", i);
      String s = prefs.getString(key, "");
      s.trim();
      if (s.length() > 0) {
        gVariants[i].active = true;
        safeCopy(gVariants[i].name, s.c_str(), sizeof(gVariants[i].name));
        anyStored = true;
      }
    }
    prefs.end();
    if (anyStored) saveListBlob(currentVarPrefsNs(), gVariants, sizeof(gVariants));
  }
...
static void loadManufacturers() {
  ...
  if (!anyStored) {
    prefs.begin(currentMfgPrefsNs(), true);
    size_t blobLen = prefs.getBytesLength(PREF_LIST_BLOB);
    const size_t legacySize = sizeof(RuntimeItem) * (LEGACY_MAX_MANUFACTURERS + 1);
    if (blobLen == legacySize) {
      RuntimeItem legacyManufacturers[LEGACY_MAX_MANUFACTURERS + 1];
      memset(legacyManufacturers, 0, sizeof(legacyManufacturers));
      if (prefs.getBytes(PREF_LIST_BLOB, legacyManufacturers, sizeof(legacyManufacturers)) == sizeof(legacyManufacturers)) {
        for (uint8_t i = 0; i <= LEGACY_MAX_MANUFACTURERS; i++) {
          gManufacturers[i] = legacyManufacturers[i];
        }
        anyStored = true;
      }
    }
    prefs.end();
    if (anyStored) saveListBlob(currentMfgPrefsNs(), gManufacturers, sizeof(gManufacturers));
  }
  if (!anyStored) {
    prefs.begin(currentMfgPrefsNs(), true);
    for (uint8_t i = 0; i <= MAX_MANUFACTURERS; i++) {
...
static void loadLanguage() {
  ...
  if (blobLen == sizeof(UiSettingsBlob)) {
    ...
    if (prefs.getBytes(PREF_UI_BLOB, &blob, sizeof(blob)) == sizeof(blob) &&
      ...
      if (qm > (uint8_t)QIDI_MODEL_Q2) qm = (uint8_t)QIDI_MODEL_PLUS4;
...
static void drawTagInfoPopup(uint8_t matID, uint8_t colID, uint8_t mfgID) {
  ...
  tft.drawCentreString(TR(STR_TAG_INFO_TITLE), TFT_W / 2, y + 6, 2);
...
static void drawReadScreen() {
  ...
  tft.drawCentreString(tagModeLabel(), TFT_W / 2, 138, 2);
  drawStatusBarFrame();
  drawStatus(tagModeLabel(), TFT_WHITE);
}
...
static void drawSetupScreen() {
  ...
  const bool hasTagInfoPage = (currentTagMode == TAGMODE_OPENSPOOL);
  ...
  } else if (hasTagInfoPage && setupPage == 2) {
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
...
static void drawKeyboardScreen() {
  ...
  if (kbMode == KB_HEX) {
    ...
    drawKeyboardKey(54 + 7 * (keyW + gap), row3Y, 46, keyH, "_", TFT_DARKCYAN);
...
static bool keyboardHandleTouch(int x, int y) {
  ...
  if (kbMode == KB_HEX) {
    ...
    if (x >= 10 && x < 54 && y >= hxRow3Y && y < hxRow3Y + hxH) {
      if (kbTargetBuffer) {
        memset(kbTargetBuffer, 0, kbTargetMaxLen + 1);
      }
      needRedraw = true;
      return true;
    }
    ...
    if (!kbForceNumeric && x >= 10 && x < 58 && y >= row4Y && y < row4Y + keyH) { kbMode = KB_NUM; needRedraw = true; return true; }
    ...
    if (kbStrictNumeric) {
      ...
      if (x >= 112 && x < 208 && y >= row2Y && y < row2Y + keyH) { if (kbTargetBuffer) kbTargetBuffer[0] = '\0'; needRedraw = true; return true; }
      ...
      if (x >= 212 && x < 296 && y >= row2Y && y < row2Y + keyH) { keyboardBackspace(); needRedraw = true; return true; }
      if (!kbForceNumeric && x >= 10 && x < 74 && y >= row3Y && y < row3Y + keyH) { kbMode = KB_UPPER; needRedraw = true; return true; }
      if (!kbForceNumeric && x >= 78 && x < 142 && y >= row3Y && y < row3Y + keyH) { kbMode = KB_LOWER; needRedraw = true; return true; }
      if (!kbForceNumeric && x >= 146 && x < 210 && y >= row3Y && y < row3Y + keyH) { kbMode = KB_NUM; needRedraw = true; return true; }
...
static void uiHandleTouch(int x, int y) {
  ...
  if (ui == UI_SETUP) {
    ...
    if (setupPage == 0) {
      ...
      if (currentTagMode == TAGMODE_QIDI && hit(36, 166, 248, 34, x, y)) {
        qidiPrinterModel = (qidiPrinterModel == QIDI_MODEL_PLUS4) ? QIDI_MODEL_Q2 : QIDI_MODEL_PLUS4;
        saveAllSetupPreferences();
        reloadModeDatabases();
        needRedraw = true;
        return;
    ...
    } else if (currentTagMode == TAGMODE_OPENSPOOL && setupPage == 2) {
  ...
  if (ui == UI_MAT_EDIT_DETAIL) {
    ...
    if (currentTagMode == TAGMODE_OPENSPOOL) {
      if (hit(120, 66, 180, 28, x, y)) { openKeyboardForBuffer(editMatName, ITEM_NAME_MAX, UI_MAT_EDIT_DETAIL); return; }
  ...
  if (ui == UI_MAT_ADD_DETAIL) {
    ...
    if (currentTagMode == TAGMODE_OPENSPOOL) {
      if (hit(120, 66, 180, 28, x, y)) { openKeyboardForBuffer(addMatName, ITEM_NAME_MAX, UI_MAT_ADD_DETAIL); return; }
  ...
  if (ui == UI_VAR_EDIT_DETAIL) {
    ...
    if (hit(70, 160, 180, 34, x, y)) { openKeyboardForBuffer(editVarName, ITEM_NAME_MAX, UI_VAR_EDIT_DETAIL); return; }
    return;
  }
  ...
  if (ui == UI_VAR_ADD_DETAIL) {
    ...
    if (hit(70, 160, 180, 34, x, y)) { openKeyboardForBuffer(addVarName, ITEM_NAME_MAX, UI_VAR_ADD_DETAIL); return; }
    return;
  }
