...
static const char* APP_VERSION = "V4.2";
...
static void fillSendTagFromQidi(uint8_t matID, uint8_t colID, uint8_t mfgID, const uint8_t* uid, uint8_t uidLen) {
  ...
  memcpy(sendTag.uid, uid, sendTag.uidLen);
}
...
static void drawToolHeadSendButton(uint8_t idx, int x, int y, int w, int h) {
  ...
  tft.setTextColor(textCol, fill);
  tft.setTextDatum(MC_DATUM);
  tft.drawString(String("TH") + String(idx + 1), x + w / 2, y + 10, 2);
  String mat = (snapToolHeadsKnown && th.known) ? String(th.material) : String("-");
  tft.drawString(trimName18(mat), x + w / 2, y + h - 12, 1);
  tft.setTextDatum(TL_DATUM);
}
...
static void drawSendSelectScreen() {
  ...
  if (sendTag.valid) {
    ...
    tft.drawString(trimName18(String(sendTag.mainType) + " " + String(sendTag.subType)), 16, 118, 2);
  ...
  drawStatusBarFrame();
  if (!readyToRead) drawStatus("Senden nicht moeglich", TFT_YELLOW);
...
static void uiHandleTouch(int x, int y) {
  ...
  if (ui == UI_MAIN) {
    if (hit(20, 50, 140, 50, x, y))  {
      ...
      readPopupMisses = 0;
      autoPanelVisible = false;
      if (wifiEnabled && WiFi.status() == WL_CONNECTED && strlen(snapmakerHost) > 0) refreshSnapmakerToolHeadInfo();
  ...
  if (ui == UI_SEND_SELECT) {
    if (hit(8, 40, 84, 28, x, y)) { clearSendTag(); ui = UI_MAIN; needRedraw = true; return; }
    if (wifiEnabled && WiFi.status() == WL_CONNECTED && strlen(snapmakerHost) > 0 &&
